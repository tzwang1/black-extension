...
//
//    The BMP format requires that each horizontal line be a multiple of 4
//    bytes. If the data itself does not have a WIDTH that is a multiple of 4,
//    then the file must be padded with a few extra bytes so that each line has
//    the appropriate length.  This information, and the corresponding
//    corrective code, was supplied by Lee Mulcahy.
//
//  Modified:
//
//    02 April 2005
...
void bmp_header1_print(unsigned short int filetype, unsigned long int filesize,
...
//
{
    ...
    //
    //  If you are doing swapping, you have to reunswap the filetype, I think,
    //  JVB 15 December 2004.
    //
    if (bmp_byte_swap) {
...
void bmp_header2_print(
    unsigned long int size, unsigned long int width, long int height,
...
//    4 bytes COLORSIMPORTANT.     Minimum number of important colors. (Can be
//    zero).
//
//  Modified:
//
...
bool bmp_header2_read(
    ifstream &file_in, unsigned long int *size, unsigned long int *width,
    long int *height, unsigned short int *planes,
    unsigned short int *bitsperpixel, unsigned long int *compression,
    unsigned long int *sizeofbitmap, unsigned long int *horzresolution,
...
//
//    Output, unsigned long int *COLORSUSED, the number of colors in the
//    palette.
//
...
void bmp_header2_write(
    ofstream &file_out, unsigned long int size, unsigned long int width,
...
void bmp_palette_print(unsigned long int colorsused, unsigned char *rparray,
                       unsigned char *gparray, unsigned char *bparray,
...
//    Input, unsigned long int COLORSUSED, the number of colors in the palette.
//
//    Input, unsigned char *RPARRAY, *GPARRAY, *BPARRAY, *APARRAY, pointers to
//    the red, green, blue and transparency palette arrays.
//
{
    int i;
    ...
    for (i = 0; i < colorsused; i++) {
        cout << setw(6) << i << "  " << setw(6) << *indexb << "  " << setw(6)
             << *indexg << "  " << setw(6) << *indexr << "  " << setw(6)
             << *indexa << "\n";
...
bool bmp_palette_read(ifstream &file_in, unsigned long int colorsused,
                      unsigned char *rparray, unsigned char *gparray,
                      unsigned char *bparray, unsigned char *aparray)
...
#define VERBOSE false
    ...
    error = bmp_header2_read(file_in, &size, &width, &height, &planes,
                             ...
                             &horzresolution, &vertresolution, &colorsused,
                             &colorsimportant);
    ...
    bmp_header2_print(size, width, height, planes, bitsperpixel, compression,
                      sizeofbitmap, horzresolution, vertresolution, colorsused,
                      colorsimportant);
    //
    //  Read the palette.
    //
    ...
    error = bmp_palette_read(file_in, colorsused, rparray, gparray, bparray,
                             aparray);
    ...
    if (bitsperpixel == 8) {
        ...
        *barray = *rarray;
        ...
        error =
            bmp_24_data_read(file_in, width, height, rarray, garray, barray);
...
bool bmp_read(char const *file_in_name, unsigned long int *width,
...
//    Output, long int *HEIGHT, the Y dimension of the image.
//
//    Output, unsigned char **RARRAY, **GARRAY, **BARRAY, pointers to the red,
//    green and blue color arrays.
//
...
//
{
    ...
    if (filetype != magic) {
        ...
        cout << "\n";
        cout << "  Instead, it is \"" << (char)(filetype / 256)
             << (char)(filetype % 256) << "\".\n";
    ...
    //
    error =
        bmp_header2_read(file_in, &size, width, height, &planes, &bitsperpixel,
                         &compression, &sizeofbitmap, &horzresolution,
...
#define VERBOSE false
    ...
    //
    error = bmp_read(file_in_name, &width, &height, &rarray, &garray, &barray);
    //
    //  Free the memory.
    //
    delete[] rarray;
...
bool bmp_write(char const *file_out_name, unsigned long int width,
               long int height, unsigned char *rarray, unsigned char *garray,
               unsigned char *barray) {
...
bool bmp_08_write(char const *file_out_name, unsigned long int width,
                  long int height, unsigned char *rarray)
...
//
{
    ...
    bmp_header1_write(file_out, filetype, filesize, reserved1, reserved2,
                      bitmapoffset);
    //
    //  Write header 2.
    //
    ...
    //  Write the palette.
    //
    bmp_palette_write(file_out, colorsused, rparray, gparray, bparray, aparray);
    //
    //  Write the data.
    //
...
bool bmp_24_write(char const *file_out_name, unsigned long int width,
                  long int height, unsigned char *rarray, unsigned char *garray,
                  unsigned char *barray)
...
//    must have a length that is a multiple of 4, or be padded so that this is
//    the case.
//
//    Thanks to Tak Fung for suggesting that BMP files should be opened with
//    the binary option.
